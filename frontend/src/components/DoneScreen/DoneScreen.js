...
export default function DoneScreen() {
    ...
    const users = Object.keys(tweetsByUser).filter(key => key !== 'profile images');

    const handlePrev = () => {
        setCurrentIndex((prevIndex) => {
            let newIndex = prevIndex === 0 ? users.length - 1 : prevIndex - 1;
            return users[newIndex] === "profile_images" ? newIndex - 1 : newIndex;
        });
    };
    
    const handleNext = () => {
        setCurrentIndex((prevIndex) => {
            let newIndex = prevIndex === users.length - 1 ? 0 : prevIndex + 1;
            return users[newIndex] === "profile_images" ? newIndex + 1 : newIndex;
        });
    };
