    ...
    <div className="max-w-[100rem] ml-auto mr-auto flex flex-col items-center">
      <div className="grid gap-3 grid-cols-4 px-3 py-4">
        {correctTiles.map((tweet) => (
          <Button
            key={tweet.id}
            className={
              "py-3 px-1 rounded-lg overflow-scroll h-20 w-13 max-w-md bg-pink-500"
            }
        ...
        {incorrectTiles.map((tweet) => (
          <Button
            ...
            className={`py-3 px-1 rounded-lg overflow-scroll h-20 w-13 max-w-md ${
              selectedTiles.includes(tweet.id) ? "bg-blue-100" : "bg-gray-100"
            }`}
            onClick={() => selectTweetTile(tweet.id)}
          >
            {tweet.text}
      ...
      <div className="">
        <div className="py-3 flex gap-2">
          <h1 className=""> Mistakes Remaining: </h1> <MistakesRemaining />
        </div>
      </div>
      <div className="flex gap-2 place-content-center">
        <Button
          className={
            "active:bg-blue-300 border border-opacity-100 px-6 py-2 rounded-full border-black hover:bg-gray-100 duration-200"
          }
        ...
        <Button
          className={
            "active:bg-blue-500 border border-opacity-100 px-6 py-2 rounded-full border-black hover:bg-gray-100 duration-200"
          }
