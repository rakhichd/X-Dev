...
export default function Main() {
  ...
  function MistakesRemaining() {
    // num of dots to show up based on how many mistakes are remaining
    return (
  ...
  const CustomizeButton = ({ width }) => {
    return (
      <button
        className={`flex flex-row items-center bg-black bg-opacity-10 p-2 rounded-[4px] mb-[6px] border border-white border-opacity-10 hover:bg-opacity-[.15] `}
        onClick={() => {
        ...
        {Object.keys(shuffledTweets).map((tweetID) => (
          <Button
            ...
            className={`py-3 px-1 rounded-lg overflow-scroll h-20 w-13 max-w-md ${
              selectedTiles.has(tweetID) ? "bg-blue-100" : "bg-gray-100"
            }`}
